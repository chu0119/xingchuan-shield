...
# Nginx default (no referer/ua)
NGINX_DEFAULT_RE = re.compile(
    r'(?P<ip>[\d.]+|[\da-fA-F:]+)'
    r'\s+\S+\s+\S+'
    r'\s+\[(?P<time>[^\]]+)\]'
    r'\s+"(?P<method>\w+)\s+(?P<path>[^\s]*?)\s*(?P<protocol>[^"]*)?"'
...
class NginxParser(BaseParser):
    ...
    def detect_format(self, sample_lines: list[str]) -> bool:
        ...
        for line in sample_lines[:10]:
            ...
            if not line:
                continue
            if NGINX_COMBINED_RE.match(line) or NGINX_DEFAULT_RE.match(line):
                matches += 1
        return matches >= 2
    ...
    def parse_line(self, line: str) -> Optional[LogEntry]:
        ...
        m = NGINX_COMBINED_RE.match(line) or NGINX_DEFAULT_RE.match(line)
        if not m:
            return None
